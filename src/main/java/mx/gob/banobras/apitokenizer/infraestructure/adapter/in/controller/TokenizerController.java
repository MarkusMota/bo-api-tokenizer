...
public class TokenizerController implements ITokenizerController {
	...
	public ResponseEntity<TokenizerResponseDTO> getToken(
		...
		try {
			...
			log.error(ConstantsToken.NULL_POINTER_EXCEPTION.getName(), enull);
			errorMessage = new ErrorMessageDTO(HttpStatus.INTERNAL_SERVER_ERROR.value(), new Date(), ConstantsToken.MSG_ERROR_500.getName());
			...
			errorMessage = new ErrorMessageDTO(HttpStatus.SERVICE_UNAVAILABLE.value(), new Date(), ConstantsToken.MSG_ERROR_503.getName());
			tokenizerResponseDTO = new TokenizerResponseDTO();
	...
	public ResponseEntity<TokenizerResponseDTO> validToken(
		...
		try {
			...
			if (tokenizerResponseDTO.getStatusCode() == 200) {
				...
				log.info("Hay un error al validar el token");
				return new ResponseEntity<>(tokenizerResponseDTO,
						HttpStatus.valueOf(tokenizerResponseDTO.getStatusCode()));
			}
	...
	private String obtenValorPorRemoteAddr(String ipAddress) throws UnknownHostException {
		...
		if (ipAddress.equals(ConstantsToken.LOCALHOST_IPV4.getName()) || ipAddress.equals(ConstantsToken.LOCALHOST_IPV6.getName())) {
			log.info("IPV4 - IPV5");
